...
def BED2TSV((srcFile, desDir)):
    dataName = path.basename(srcFile).replace('.bed', '')
    tsvDir = path.join(desDir, dataName)
    cmd = 'makeTagDirectory %s %s -format bed -genome %s -checkGC' % (tsvDir, srcFile, genome)
    print cmd
    os.system(cmd)
...
if __name__ == '__main__':
    if len(sys.argv)<4:
        print 'Usage:\n\tS1.BED2TSV.py [MaxProc] [desDir] [srcDir]'
        sys.exit(1)
...
##            for fg in fgL]
    ...
    argL = [[bedFile, desDir] for bedFile in bedL]
