...
##    print srcDir, dataName
    desForFile = path.normpath(path.join(desDir, '%s.for.wig' % dataName))
    print desForFile
    gsBIN2WIG(srcForL, desForFile, dataName+'.forward', binSize)
    desRevFile = path.normpath(path.join(desDir, '%s.rev.wig' % dataName))
    print desRevFile
    gsBIN2WIG(srcRevL, desRevFile, dataName+'.reverse', binSize)
